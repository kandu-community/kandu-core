...
class ChoicesMixin(object):
    ...
    def get_django_args(self):
        ...
        choices = django_args.pop('choices', [])
        if isinstance(choices, list):
            django_args['choices'] = [ (generate_name(verbose), verbose) for verbose in choices ]
        else:
            django_args['choices'] = [ (value, text) for value, text in choices.iteritems() ]
