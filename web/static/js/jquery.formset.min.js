...
;(function($) {
    $.fn.formset = function(opts)
    {
            ...
            checkActionButtons = function() {
                ...
                if (options.min_num > 0) {
                    if (formCount <= options.min_num) {
                        $$formsetContainer.find('.'+options.deleteCssClass.split(' ')[0]).hide()
                    } else {
                        $$formsetContainer.find('.'+options.deleteCssClass.split(' ')[0]+':gt('+(options.min_num-1)+')').show()
                    }
                }
                if (max_num > 0) {
                    if (formCount >= max_num) {
                        $$formsetContainer.find('.'+options.addCssClass.split(' ')[0]).hide()
                    } else {
                        $$formsetContainer.find('.'+options.addCssClass.split(' ')[0]).show()
                    }
                }
            };
        ...
        $$.each(function(i) {
            var row = $(this),
                del = row.find('div.form-group:has(input:checkbox[id $= "-DELETE"])');
            if (del.length) {
                // If you specify "can_delete = True" when creating an inline formset,
                // Django adds a checkbox to each form in the formset.
                // Replace the default checkbox with a hidden field:
                delCheckbox = del.find('input:checkbox[id $= "-DELETE"]');
                del.before('<input type="hidden" name="' + delCheckbox.attr('name') +'" id="' + delCheckbox.attr('id') +'" />');
                del.remove();
            }
        ...
        if ($$.length) {
            ...
            addButton.click(function() {
                ...
                if (typeof manage_visibility_for_form != "undefined") { // NOTE: see visibility_dependencies_tag.html
				    manage_visibility_for_form(options.prefix);
				}

                row.trigger('inline:created');
